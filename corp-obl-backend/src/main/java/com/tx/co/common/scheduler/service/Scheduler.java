 ...
 */
@Service
public class Scheduler extends UpdateCacheData implements IUserManagementDetails {
					...
					if (sendFirstNotification || sendNextNotification) {
						...
						if (null != expirationOffice) {
							...
							if (expiration.getExpirationClosableBy() == EXPIRATION_CLOSABLEBY_ALL) {
								...
								Mail mail = new Mail(beneficiaryEmails, 
										providerEmails, 
										"", 
										getTextFromTranslation(EXPIRATION_SUBJECT_EXPIRED, username), 
										getTextFromTranslation(EXPIRATION_TEXT_EXPIRED, username),
										MAIL_TEMPLATE_EXPIRED);
								...
								if (null != user) {
									...
									emailService.sendMailUsingTemplate(new Mail(email, 
											providerEmails, 
											"", 
											getTextFromTranslation(EXPIRATION_SUBJECT_EXPIRED, username), 
											getTextFromTranslation(EXPIRATION_TEXT_EXPIRED, username),
