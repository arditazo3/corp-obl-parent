...
public interface TaskTemplateRepository extends CrudRepository<TaskTemplate, Long>  {
	...
	@Query("select tt from TaskTemplate tt where tt.enabled <> 0 and tt.isRapidConfiguration <> 1 order by tt.description asc ")
	List<TaskTemplate> findAllOrderNotRapidConfigurationByDescriptionAsc();
